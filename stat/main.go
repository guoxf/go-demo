...
func main() {
	var filePath string
	flag.StringVar(&filePath, "filepath", "./game.log", "log file path")
	flag.Parse()
	...
	wf, err := createFileOfTrunc(filePath + ".append")
	if err != nil {
		panic(err)
	}
	defer wf.Close()

	reg, err := regexp.Compile(`----Handle End Message: (.+?)  elasped:  (.+?)ms----`)
	if err != nil {
		panic(err)
	}
	...
	for {
		...
		if _, ok := m[result[1]]; ok {
			m[result[1]] = append(m[result[1]], result[2])
		} else {
			m[result[1]] = []string{result[2]}
		}
	}
	array := make([]StatInfo, 0)
	for k, v := range m {
		var info StatInfo
		info.Protocol = k
		info.Count = len(v)
		info.ExcuteTimes = v
		info.Sum, info.Avg, info.Max, info.Min = avg(v)
		array = append(array, info)
	}
	sortByCount(wf, array)
	sortBySum(wf, array)
	sortByAvg(wf, array)
	sortByMax(wf, array)
	var str string
...
func (s StatInfo) String() string {
	return fmt.Sprintf("%s\t%-6d\t%-6.2f\t%-15.2f\t%-6.2f\t%-6.2f\t\n", s.Protocol, s.Count, s.Avg, s.Sum, s.Max, s.Min)
}

func avg(array []string) (float64, float64, float64, float64) {
	...
	return sum, sum / float64(len(array)), max, min
}

const (
	HERDER = "协议号\t\t数量\t平均值\t\t总耗时\t\t最大耗时\t\t最小耗时\t\n"
    
)
var HERDERS=fmt.Sprintf("%-5s\t%-3s\t%-3s\t%-8s\t%s\t%s\t\n","协议号","数量","平均值","总耗时","最大耗时","最小耗时")
func sortByCount(wf *os.File, array []StatInfo) {
	Sort(array[:], func(i, j int) bool {
		return array[i].Count > array[j].Count
	})
	result := fmt.Sprint("按执行数量排序:\n", HERDERS, array)
	fmt.Println(result)
	wf.WriteString("\n")
	wf.WriteString(result)
}

func sortBySum(wf *os.File, array []StatInfo) {
	Sort(array[:], func(i, j int) bool {
		return array[i].Sum > array[j].Sum
	})
	result := fmt.Sprint("按总耗时排序:\n", HERDERS, array)
	fmt.Println(result)
	wf.WriteString("\n")
	wf.WriteString(result)
}

func sortByAvg(wf *os.File, array []StatInfo) {
	Sort(array[:], func(i, j int) bool {
		return array[i].Avg > array[j].Avg
	})
	result := fmt.Sprint("按平均耗时排序:\n", HERDERS, array)
	fmt.Println(result)
	wf.WriteString("\n")
	wf.WriteString(result)
}

func sortByMax(wf *os.File, array []StatInfo) {
	Sort(array[:], func(i, j int) bool {
		return array[i].Max > array[j].Max
	})
	result := fmt.Sprint("按最大耗时排序:\n", HERDERS, array)
	fmt.Println(result)
	wf.WriteString("\n")
	wf.WriteString(result)
}
